class ClientProfilesController < ApplicationController
  ...
  def create
    ...
    if @client_profile.save
      ...
      if response.status != 200 #500 (API down), 401 (erro de autenticação), 422 (validação dos dados)
      #  response_body = {name: @client_profile.full_name, cpf: @client_profile.cpf, token: SecureRandom.alphanumeric(20)}
      #  @client_profile.token = response_body[:token]
      elsif response.status == 200
        response_body = JSON.parse(response.body, simbolize_names: true)
        @client_profile.token = response_body[0]["token"]
        @client_profile.update(params.require(:client_profile).permit(:token))
